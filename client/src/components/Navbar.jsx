...
const changeRole = async() =>{
  ...
  return (
    ...
    className={`flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 text-gray-600 border-b border-borderColor relative transition-all ${location.pathname === '/' && "bg-light"}`}>
      ...
      className='text-3xl text-primary font-bold'>Drive<span className='text-black'>Easy</span></motion.h1>
      </Link>
      <div className={`max-sm:fixed max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor right-0 flex flex-col sm:flex-row items-start sm:items=center gap-4 sm:gap-8 max-sm:p-4 transition-all duration-400 z-50 ${location.pathname === '/' ? "bg-light" : "bg-white"} ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
